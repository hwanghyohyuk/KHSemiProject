		...
		<form action="/studyhub/gboardupdate" method="post">
			<div class="form-group">
				<label for="title">제목</label> <input type="text"
					...
					aria-describedby="emailHelp" value=<%=gBoard.getTitle()%>>
			</div>
			<div class="range">
				<label for="Content">공개범위</label> <label class="radio-inline">
					...
					name="accessno" id="inlineRadio2" value="3" checked> 그룹원공개
				</label>
			</div>
			<div class="form-group">
				<label for="Content">내용</label>
				<textarea class="form-control" id="Textarea" rows="15"
					name="content"><%=gBoard.getContent()%></textarea>
			</div>
			<input type="submit" value="수정하기" class="btn btn-info" id="btns">
			<a href="/studyhub/gboardpreview?groupno=<%=group.getGroupNo()%>">취소</a>
			<input type="hidden" name="gBoardNo" value="<%=gBoard.getgBoardNo() %>">			
